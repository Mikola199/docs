...
const latestVersionByPlan = new Map(
  Object.values(allVersions).map((versionObject) => [
    versionObject.plan,
    allVersions[versionObject.latestVersion].version,
  ])
)
...
export function getPathWithoutVersion(href) {
  ...
  return allVersions[versionFromPath] ? href.replace(`/${versionFromPath}`, '') : href
}
...
export function getVersionStringFromPath(href) {
  ...
  const latestVersionFromPlan = latestVersionByPlan.get(versionFromPath)
  if (latestVersionFromPlan) {
    return latestVersionFromPlan
  }
